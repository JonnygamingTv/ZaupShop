...
namespace ZaupShop.Commands
{
    public class CommandCost : IRocketCommand
    {
        ...
        public void Execute(IRocketPlayer caller, string[] command)
        {
            ...
            {
                UnturnedChat.Say(caller, ZaupShop.instance.Translate("cost_command_usage"));
                return;
            }
            ...
            switch (command[0])
            {
                case "v":
                    ...
                    {
                        UnturnedChat.Say(caller, ZaupShop.instance.Translate("could_not_find", command[1]));
                        return;
                    }
                    ...
                    if (cost <= 0m)
                    {
                        UnturnedChat.Say(caller, ZaupShop.instance.Translate("error_getting_cost", name));
                    }
                    else
                    {
                        UnturnedChat.Say(caller, ZaupShop.instance.Translate("vehicle_cost_msg", name, cost.ToString(CultureInfo.CurrentCulture), ZaupShop.instance.Configuration.Instance.UconomyCurrencyName));
                    }
                    break;
                default:
                    name = null;
                    ...
                    {
                        UnturnedChat.Say(caller, ZaupShop.instance.Translate("could_not_find", command[0]));
                        return;
                    }
                    ...
                    {
                        UnturnedChat.Say(caller, ZaupShop.instance.Translate("item_cost_msg", name, cost.ToString(CultureInfo.CurrentCulture), ZaupShop.instance.Configuration.Instance.UconomyTableName, bbp.ToString(CultureInfo.CurrentCulture), ZaupShop.instance.Configuration.Instance.UconomyTableName));
                    }
                    break;
            }
        }
