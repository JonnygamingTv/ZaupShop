...
namespace ZaupShop.Commands
{
    public class CommandSell : IRocketCommand
    {
        ...
        public void Execute(IRocketPlayer caller, string[] command)
        {
            ...
            {
                UnturnedChat.Say(caller, ZaupShop.instance.Translate("sell_command_usage"));
                return;
            }
            byte amttosell = 1;
            if (command.Length > 1)
            ...
            {
                UnturnedChat.Say(caller, ZaupShop.instance.Translate("sell_items_off"));
                return;
            }
            ...
            {
                UnturnedChat.Say(caller, ZaupShop.instance.Translate("not_enough_items_sell", amttosell.ToString(), asset.itemName));
                return;
            }
            if (asset.amount > 1)
            {
                ...
                {
                    UnturnedChat.Say(caller, ZaupShop.instance.Translate("not_enough_ammo_sell", asset.itemName));
                    return;
                }
            }
            ...
            if (price <= 0.00m)
            {
                UnturnedChat.Say(caller, ZaupShop.instance.Translate("no_sell_price_set", asset.itemName));
                return;
            }
            byte quality = 100;
            decimal addmoney = 0;
            ...
            UnturnedChat.Say(caller, ZaupShop.instance.Translate("sold_items", amt, asset.itemName, addmoney, ZaupShop.instance.Database.GetBalance(player.Id), balance, ZaupShop.instance.Database.GetBalance(player.Id)));
        }
    }
}